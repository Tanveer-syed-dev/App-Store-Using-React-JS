...
class AppStore extends Component {
  ...
  updateActiveTabId = value => {
    this.setState({activeTabId: value})
  }

  render() {
    const {activeTabId, userSearch} = this.state
    const filteredAppList = this.getFilterAppList()
    // const userSearchInput = this.userSearchInput()
    const searchResults = filteredAppList.filter(
      eachSearchAppItem =>
        eachSearchAppItem.appName
          .toLowerCase()
          .includes(userSearch.toLowerCase()) &&
        eachSearchAppItem.category === activeTabId,
    )
    return (
      <div className="app-container">
        <div className="app-store-container">
          ...
          <ul className="app-list-container">
            {searchResults.map(eachAppList => (
              <AppItem key={eachAppList.appId} appListDetails={eachAppList} />
            ))}
          </ul>
        </div>
      </div>
